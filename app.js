...
const hasStatusAndPriorityProperty = (requestQuery) => {
    return (requsetQuery.status!==undefined && requestQuery.priority!==undefined);
};
...
app.get("/todos/", async (request, response) => {
    ...
    switch(true) {
        ...
        case hasStatusProperty(request.query):
            if (status==="TO DO" || status==="MEDIUM" || status==="DONE") {
                getTodoQuery = `SELECT * FROM todo WHERE status = '${status}';`;
                    data = await db.all(getTodoQuery);
                    response.send(data.map((eachQuery) => responseObject(eachQuery)));
            }else{
...
app.get("/agenda/", async (request, response) => {
    ...
    if isMatch(date, "yyyy-MM-dd") {
        ...
        response.send(getTodoQuery.map((eachQuery) => responseObject(eachQuery)))
